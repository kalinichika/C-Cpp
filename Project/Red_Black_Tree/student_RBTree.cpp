...
using std::endl;
    ...
    void RedBlackTree<T>::Insert(T value){
        ...
        // Если в дереве есть элементы
            ...
            while(node){
                ...
                else return; // Если узел со значением value уже есть в дереве
            }
            // 2. Создаем новый элемент и связываем его с родителем
            node = NewNode(value);
            node->parent = par;
            if (flag == 'L')     par->left = node;
            else if(flag == 'R') par->right = node;
            //  3. Балансировка дерева при условии, что отец вставки - красный.
            if(par->color) BalanceInsert(node);
        }
        // В любом случае после успешной вставки красим корень дерева в черный цвет
    ...
    template <typename T>
    void RedBlackTree<T>::Show(const Node<T>* node, int high, char dir)const{ //high - высота, dir - значок
        int n;
        if(!node) return;
    ...
    void RedBlackTree<T>::BalanceInsert(Node<T>* node){
        ...
        // Пока вершина x не корень и ее родитель - красный
        while((node != RBT_root) && par->color){
            int flag = 0;
            // Если дядя вставки - красный
            // Отец и дядя -> красные, дед -> черный. Далее проверка для деда
            if(par->parent->left && par->parent->right &&
                    par->parent->left->color && par->parent->right->color){
            ...
            // Если проверяемя вершина - корень или ее родитель - корень
            if((node == RBT_root) || (par == RBT_root)) return;

            // Если дядя - черный
            if( !par->parent->left || !par->parent->left->color ){
                // Если вершина и ее отец - одинаковые дети -> делаем вращение
            ...
            if( !flag && ( !par->parent->right || !par->parent->right->color )){
                // Если вершина и ее отец - одинаковые дети -> делаем вращение
            ...
            if((X == par->left)||(par->left==0 && X->value==T())){
                ...
                if((par->right) && par->right->color){
                    ...
                    if((!par->right->right && !par->right->left) ||
                            (!par->right->right && !par->right->left->color) ||
                            (!par->right->left && !par->right->right->color) ||
                            ((par->right->left) && (par->right->right) &&
                             !par->right->left->color && !par->right->right->color))
                    {
                        par->right->color = Red;
                        X = par;
                        par = X->parent;
                    ...
                    else if(par->right->right && !par->right->right->color){
                        par->right->color = Red;
                ...
                if((par->left) && par->left->color){
                    ...
                    if((!par->left->right && !par->left->left) ||
                       ...
                       ((par->left->left) && (par->left->right) &&
                        !par->left->left->color && !par->left->right->color))
                    {
                        par->left->color = Red;
                        X = par;
                        par = X->parent;
